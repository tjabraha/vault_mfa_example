...
vault_token = 's.7osR7zASDqNNfhvFxxZPbSZz'
...
vault_token_headers = {'X-Vault-Token': vault_token}
hash_data = {"input": password}

# Http communication with Vault via the requests python module
hash_the_password = requests.post(hash_algorithm_url_path, headers=vault_token_headers, data=hash_data)
pass_hash = json.loads(hash_the_password.content)
get_password_data = requests.get(secret_url_path, headers=vault_token_headers)
...
if password_from_user == password_from_vault:
    ...
    check_if_token_is_valid = requests.put(totp_endpoint_url, headers=vault_token_headers, data=mfa_data)
